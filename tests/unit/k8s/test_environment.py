import unittest
from k8svimdriver.k8s.environment import K8sDeploymentLocationTranslator, K8sDeploymentLocation, K8S_SERVER_PROP, K8_CERT_AUTH_DATA_PROP, K8_CLIENT_CERT_DATA_PROP, K8_CLIENT_KEY_DATA_PROP
...
class TestK8sDeploymentLocationTranslator(unittest.TestCase):
    ...
    def test_from_deployment_location_missing_properties(self):
        ...
        self.assertEqual(str(context.exception), 'Deployment Location managed by the K8s VIM Driver must specify a property value for \'{0}\''.format(K8_CERT_AUTH_DATA_PROP))

        with self.assertRaises(ValueError) as context:
            translator.from_deployment_location({'name': 'testdl', 'properties': {
                K8S_SERVER_PROP: 'http://10.220.217.8:8443',
                K8_CERT_AUTH_DATA_PROP: 'fhudgf'
        }})
        self.assertEqual(str(context.exception), 'Deployment Location managed by the K8s VIM Driver must specify a property value for \'{0}\''.format(K8_CLIENT_CERT_DATA_PROP))

        with self.assertRaises(ValueError) as context:
            translator.from_deployment_location({'name': 'testdl', 'properties': {
                K8S_SERVER_PROP: 'http://10.220.217.8:8443',
                K8_CERT_AUTH_DATA_PROP: 'fhudgf',
                K8_CLIENT_CERT_DATA_PROP: 'dgysfyds'
        }})
        self.assertEqual(str(context.exception), 'Deployment Location managed by the K8s VIM Driver must specify a property value for \'{0}\''.format(K8_CLIENT_KEY_DATA_PROP))
    ...
    def test_from_valid_deployment_location(self):
        ...
        k8s_location = translator.from_deployment_location({'name': 'testdl', 'properties': {
                ...
                K8_CERT_AUTH_DATA_PROP: 'fhudgf',
                K8_CLIENT_CERT_DATA_PROP: 'dgysfyds',
                K8_CLIENT_KEY_DATA_PROP: 'fhudgfu'
        }})
